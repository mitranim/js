...
export class StringReporter {
  constructor(pad) {this.pad = l.reqStr(pad)}
...
export function tsMicro(val) {return `${(l.reqFin(val) * 1000).toFixed(3)} μs`}
...
export function ok(val) {
  if (val === true) return
  throw new AssertError(`expected true, got ${l.show(val)}`)
}

// Asserts that the given value is exactly `false`. Otherwise throws `AssertError`.
export function no(val) {
  if (val === false) return
  throw new AssertError(`expected false, got ${l.show(val)}`)
}
