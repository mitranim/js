...
export function readText(path) {return Deno.readTextFileSync(reqPath(path))}

export function readTextOpt(path) {
...
export function readTextCreate(path) {
  try {return readText(path)}
  catch (err) {
...
export function writeText(path, val, opt) {
  return Deno.writeTextFileSync(reqPath(path), l.reqStr(val), opt)
}

export function stat(path) {return Deno.statSync(reqPath(path))}
...
export function create(path) {return Deno.createSync(reqPath(path))}

export function touch(path) {
  const info = statOpt(path)
  if (!info) {
    create(path)
    return
  }
