...
t.test(function test_Words() {
  ...
  t.eq(src().sente(), s.Words.from([`One`, `two`, `three`]))
  t.eq(src().camel(), s.Words.from([`one`, `Two`, `Three`]))

  t.is(src().lower().space(), `one two three`)
  t.is(src().lower().snake(), `one_two_three`)
  t.is(src().lower().kebab(), `one-two-three`)
  t.is(src().lower().solid(), `onetwothree`)

  t.is(src().upper().space(), `ONE TWO THREE`)
  t.is(src().upper().snake(), `ONE_TWO_THREE`)
  t.is(src().upper().kebab(), `ONE-TWO-THREE`)
  t.is(src().upper().solid(), `ONETWOTHREE`)

  t.is(src().title().space(), `One Two Three`)
  t.is(src().title().snake(), `One_Two_Three`)
  t.is(src().title().kebab(), `One-Two-Three`)
  t.is(src().title().solid(), `OneTwoThree`)

  t.is(src().sente().space(), `One two three`)
  t.is(src().sente().snake(), `One_two_three`)
  t.is(src().sente().kebab(), `One-two-three`)
  t.is(src().sente().solid(), `Onetwothree`)

  t.is(src().camel().space(), `one Two Three`)
  t.is(src().camel().snake(), `one_Two_Three`)
  t.is(src().camel().kebab(), `one-Two-Three`)
  t.is(src().camel().solid(), `oneTwoThree`)

  t.is(src().lowerSpace(), `one two three`)
  t.is(src().upperSpace(), `ONE TWO THREE`)
  t.is(src().titleSpace(), `One Two Three`)
  t.is(src().senteSpace(), `One two three`)
  t.is(src().camelSpace(), `one Two Three`)
