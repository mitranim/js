...
/*
Allows to compare IO performance and overall build performance with
`cmd_doc.mjs`, which uses synchronous IO. TODO dedup shared code.
*/
...
class Pkg extends u.Strict {
  ...
  get readmeSrcText() {return io.readFileText(this.readmeSrcPath).then(a.trim)}
...
class Feat extends u.Strict {
  ...
  get codeText() {return io.readFileText(this.codePath)}
  ...
  get testText() {return io.readFileText(this.testPath)}
...
class Ident extends u.Strict {
  ...
  get docSrcText() {return io.readFileTextOpt(this.docPath).then(a.trim)}
